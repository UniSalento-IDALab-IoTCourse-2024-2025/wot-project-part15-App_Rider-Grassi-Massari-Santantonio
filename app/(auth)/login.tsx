...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    if (!cleanUsername || !cleanPassword) return Alert.alert("Errore", "Inserisci i dati");
    
    setLoading(true);
 
    const result = await loginUser({ username: cleanUsername, password: cleanPassword });
    setLoading(false);
    ...
    if (result.success && result.data) {
      ...
      if (result.data.role !== 'RIDER') {
        Alert.alert("Errore", "Accesso riservato ai riders (Tu sei: " + result.data.role + ")");
        return;
      }
  ...
  return (
    <View className="flex-1 justify-center px-6 bg-white">
      <Text className="text-3xl font-bold text-center mb-8 text-slate-800">FastGo Rider</Text>
